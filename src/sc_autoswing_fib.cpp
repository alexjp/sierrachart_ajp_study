#include "sc_autoswing_fib.h"

void autoswing_set_defaults(SCStudyInterfaceRef sc) {
    ...
    sc.Input[_AUTO_SWING_I_EXT_TOP].Name = "External Band Top";
    sc.Input[_AUTO_SWING_I_EXT_TOP].SetStudySubgraphValues(6, 0);

    sc.Input[_AUTO_SWING_I_EXT_MIDDLE].Name = "External Band Middle";
    sc.Input[_AUTO_SWING_I_EXT_MIDDLE].SetStudySubgraphValues(6, 0);

    sc.Input[_AUTO_SWING_I_EXT_BOTTOM].Name = "External Band Bottom";
    sc.Input[_AUTO_SWING_I_EXT_BOTTOM].SetStudySubgraphValues(6, 0);
	...
	sc.Input[_AUTO_SWING_I_USE_EXT_ATR].Name = "Use External ATR Study";
	sc.Input[_AUTO_SWING_I_USE_EXT_ATR].SetInt(1);
...
//void autoswing_fib_volumebyprice(SCStudyInterfaceRef sc, int & sg_direction) {
//	autoswing_fib_zone_swing_tool_remove(sc);
...
void autoswing_set_atr_tick_offset(SCStudyInterfaceRef sc, int index_end) {
    if (index_end > 0 && sc.Input[_AUTO_SWING_I_USE_EXT_ATR].GetInt() > 0) {
		...
		if (atr_input.is_open()) {
			int atr;
			atr_input >> atr;
			sc.Input[_AUTO_SWING_I_TICK_OFFSET].SetInt(atr);
			atr_input.close();
...
void autoswing_main_iteration(SCStudyInterfaceRef sc, int index, int prev_index) {
	...
	//if (_date_start_index == 0) return;
	//SCString log;
	//sc.AddMessageToLog(log.Format("%d", _date_start_index), 1);
    ...
    if (index < _date_start_index || index < sc.GetPersistentInt(_AUTO_SWING_P_START_INDEX) || SwingStart.GetTime() == SwingEnd.GetTime()) {
		...
		//SCString log;
		//sc.AddMessageToLog(log.Format("cleaning: %d", index), 1);
...
SCSFExport scsf_Auto_Swing(SCStudyInterfaceRef sc)
{
    ...
    if (sc.UpdateStartIndex == 0) {
        ...
        if (file.good()) {
            ...
            int max_stop_distance, vwap_length, atr_length, ma_type, repaint, show_study, alert, atr_ret_ma, draw_atr, use_atr, use_ext_atr_study;
            file >> min_fib >> entry_fib >> max_fib >> max_stop_distance >> atr_mult_limit >> atr_mult_stop >> vwap_length >> atr_length >> ma_type >> multiplier >> atr_ret_ma >> repaint >> show_study >> alert >> use_atr >> draw_atr >> use_ext_atr_study;
            ...
            sc.Input[_AUTO_SWING_I_USE_EXT_ATR].SetInt(use_ext_atr_study);
    ...
    {
        int atr_length = sc.Input[_AUTO_SWING_I_ATR_LENGTH].GetInt();

        if (atr_length < 1) {
            SCFloatArray topArray, bottomArray, middleArray;
            SCInputRef i_ext_top = sc.Input[_AUTO_SWING_I_EXT_TOP];
            SCInputRef i_ext_bottom = sc.Input[_AUTO_SWING_I_EXT_BOTTOM];
            SCInputRef i_ext_middle = sc.Input[_AUTO_SWING_I_EXT_MIDDLE];

            if (sc.GetStudyArrayUsingID(i_ext_top.GetStudyID(), i_ext_top.GetSubgraphIndex(), topArray) &&
                    sc.GetStudyArrayUsingID(i_ext_bottom.GetStudyID(), i_ext_bottom.GetSubgraphIndex(), bottomArray) &&
                    sc.GetStudyArrayUsingID(i_ext_middle.GetStudyID(), i_ext_middle.GetSubgraphIndex(), middleArray)) {
                sc.Subgraph[_AUTO_SWING_SG_ATR_MA][sc.Index] = middleArray[sc.Index];
                sc.Subgraph[_AUTO_SWING_SG_ATR_TOP][sc.Index] = topArray[sc.Index];
                sc.Subgraph[_AUTO_SWING_SG_ATR_BOTTOM][sc.Index] = bottomArray[sc.Index];
            }
        }
        else {
            sc.ATR(sc.BaseDataIn, sc.Subgraph[_AUTO_SWING_SG_ATR_VALUE], atr_length, sc.Input[_AUTO_SWING_I_ATR_TYPE].GetMovAvgType());
